...
maxZ = 100 #initialize max atomic number to calculate up to
Z = 1 #initialize minumim atomic number to start calculating at
globalmaxBindingEnergy = -100

print('Z, A - Where strongest binding energy occurs')
while Z < (maxZ+1):
    maxBindingEnergyPerNucleon = -1e5 #initialize max binding E per nucleon to some very small number
    for s in range(1,4):
        A = Z*s
        ...
        if maxBindingEnergyPerNucleon < bindingEnergyPerNucleon:
            ...
            strongestA = A
        if globalmaxBindingEnergy < bindingEnergyPerNucleon:
            globalmaxBindingEnergy = maxBindingEnergyPerNucleon
            strongestZ = Z
    print(Z, strongestA)    
    Z+=1
    
print("The maximum binding energy occurs at Z = ", strongestZ)